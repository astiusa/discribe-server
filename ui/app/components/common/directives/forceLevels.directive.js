...
define(['bower_components/d3/d3.min'], function(d3) {
    ...
    return {forceLevels: deps.concat(factory)};
    ...
    function factory($timeout, $window) {
        return {
            ...
            templateUrl: 'app/components/common/views/forceLevels.html',
            link: function(scope, element) {
                ...
                var forceColor = d3.scale.ordinal()
                    .range(["gray","steelblue","red","gray"])
                    .domain([0, 1, 2, 3]);
                ...
                svg.append("defs").append("clipPath")
                    .attr("id", "clip")
                    .append("rect")
                    .attr("class", "forceLevels clipPath")
                    .attr("width", width + margin.left + margin.right)
                    .attr("height", height + margin.top + margin.bottom);
                ...
                var dataArea = chart.append("g")
                    .attr("class", "forceLevels area");

                chart.append("g")
                    .attr("class", "forceLevels x axis")
                    .attr("transform", "translate(0," + height + ")")
                    .call(xAxis);

                chart.append("g")
                    .attr("class", "forceLevels y axis")
                    .call(yAxis);

                chart.append("g")
                    .attr("class", "forceLevels x brush")
                    .call(brush)
                    .selectAll("rect")
                    .attr("y", -6)
                ...
                chart.append("text")
                    .attr("class", "forceLevels no-items")
                    .attr("text-anchor", "middle")
                    .text("No Entities found")
                    .style("visibility", "hidden");

                var showMsg = function(msg) {
                    d3.select("text.forceLevels.no-items")
                        .attr("dx", width/2)
                        .attr("dy", height/2)
                        .text(msg)
                ...
                var update = function(showLoading) {
                    ...
                    var data = [];
                    scope.forceStats.query(scope.timeSpan).then(function(result) {
                        angular.forEach(result, function (counts, timestamp) {
                            var y0 = 0;
                            var forceCounts = counts.map(function (count, forceId) {
                                return {
                                    forceId: forceId, y0: y0, y1: y0 += count
                                };
                            });
                            ...
                            data.push({
                                ...
                                counts: forceCounts,
                                totalCount: forceCounts[forceCounts.length - 1].y1,
                                timestamp: timestamp
                            });
                        });

                        //var totalCount = d3.max(data.map(function(d) { return d.totalCount; }));
                        var totalCount = d3.max(data, function (d) {
                            return d.totalCount;
                        });
                        ...
                        //var barWidth = xScale(new Date((scope.timeSpan.startTimestamp+scope.forceStats.interval)*1000)) -
                        //    xScale(new Date(scope.timeSpan.startTimestamp*1000));
                        var barWidth = width * scope.forceStats.interval / scope.timeSpan.duration;
                        barWidth = (barWidth < 5) ? 5 : barWidth;

                        var timestampCounts = dataArea.selectAll(".timestampCounts")
                            .data(data, function (d) {
                                return d.timestamp + "-" + totalCount + "-" + scope.forceStats.interval;
                            });
                ...
                var _watchFilterUpdateFirst = false;
                ...
                scope.$watch(function(){
                    ...
                    width = parentWidth - margin.left - margin.right;
                    d3.select("svg.forceLevels").attr("width", width + margin.left + margin.right);
                    d3.select("forceLevelsclipPath").attr("width", width);
