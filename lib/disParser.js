...
disParser.parse = function(buffer, filterExpr, fields, hasGlobalHeader) {
    ...
    var self = {
        ...
        bufferView: null,           // typed buffer view
    ...
    var isValidUdpPacket = function() {
        ...
        try {
            ...
            var dv = self.bufferView.map(self.bufferIndex, self.pduLE);
    ...
    self.parsePdu = function() {
        ...
        var protocolVersion = dv.getUint8(0);
        ...
        var pdu = dv.pduView(pduType, protocolVersion);
        if (pdu) {
            // Merge second amd microsecond timestamps from packet header
            pdu.id = self.currentPacketHeader.ipSrcAddress+'-'+
                self.currentPacketHeader.ipDstPort+'-'+
                self.currentPacketHeader.timestampSeconds+'-'+
                self.currentPacketHeader.timestampMicroseconds;
    ...
    self.parsePacketHeader = function() {
        if ((self.buffer.length -self.bufferIndex) >= PACKET_HEADER_LENGTH) {
            ...
            var dv = self.bufferView.map(self.bufferIndex, self.pcapLE);
    ...
    self.parseGlobalHeader = function() {
        ...
        if (self.buffer.length >= GLOBAL_HEADER_LENGTH) {
            ...
            var dv = self.bufferView.map(0, self.pcapLE);
    ...
    try {
        ...
        if (filterExpr && filterExpr.length>0) {
            self.filter = new Filter(filterExpr);
        }

        if (fields && fields.length>0) {
            self.formatter = new Formatter(fields);
        } else {
            d.reject(new (error('No fields specified')));
        }
        self.buffer = buffer;
        self.bufferIndex = 0;
        self.bufferView = bv.dataView(self.buffer, self.bufferIndex, self.pcapLE);

        self.parser = hasGlobalHeader ? self.parseGlobalHeader : self.parsePacketHeader;

        while (self.parser.call(this)) {}

        log.info("pdu's processed: "+self.pduCount+", packets:"+self.packetCount);
